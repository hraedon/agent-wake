...
2. Secret-driven: when ``AGENT_WAKE_FORBIDDEN_IDENTIFIERS`` is set (a
...
def _resolve_identifiers() -> frozenset[str] | None:
    ...
    """
    raw = os.environ.get("AGENT_WAKE_FORBIDDEN_IDENTIFIERS", "")
    if not raw.strip():
        print(
            "AGENT_WAKE_FORBIDDEN_IDENTIFIERS is empty or unset; skipping identifier gate.",
            file=sys.stderr,
        )
        return None
    ...
    if not identifiers:
        print(
            "AGENT_WAKE_FORBIDDEN_IDENTIFIERS contained no usable identifiers (minimum "
