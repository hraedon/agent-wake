# Regista webhook → agent-wake

This guide shows how to connect regista's webhook push delivery to
agent-wake so that workflow events wake your agent session.

## Prerequisites

- Regista running with a Postgres database
...
```bash
export REGISTA_WEBHOOK_SECRET=<hex-from-above>
```

### 2. Configure agent-wake

Add a `regista` source to `~/.config/agent-wake/config.json`:
...
### 3. Register the webhook in regista

```python
import os
from regista import Regista

sub = Regista("postgresql://user:pass@localhost:5432/mydb")
sub.register_webhook(
    url="http://127.0.0.1:8788/",
    headers={
        "X-AgentWake-Source": "regista",
    },
    sign_secret=os.environ["REGISTA_WEBHOOK_SECRET"].encode(),
...
Transition a work item in regista. The daemon receives the event,
...
the reply to the source's `callback_url`. For regista, this is
typically the sidecar's reply endpoint.

## Troubleshooting

- **403 from daemon**: Check that `REGISTA_WEBHOOK_SECRET` matches
  in both regista's webhook registration and agent-wake's config.
- **`no_subscriber`**: The adapter isn't connected or isn't subscribed
  to the `regista` source. Check the adapter's hello handshake logs.
